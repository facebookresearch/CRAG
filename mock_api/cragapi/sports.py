...
class SoccerKG:
    def __init__(self, file_name="soccer_team_match_stats.pkl"):
        ...
        team_match_stats = pd.read_pickle(
            os.path.join(KG_BASE_DIRECTORY, "sports", file_name)
        )
        self.team_match_stats = team_match_stats[
            team_match_stats.index.get_level_values("league").notna()
        ]
    ...
    def get_games_on_date(self, date_str, soccer_team_name=None):
        ...
        if soccer_team_name is None:
            ...
            filtered_df = self.team_match_stats.loc[
                (slice(None), slice(None), soccer_team_name, slice(None)), :
            ]
        if len(parts) == 3:
            # date
            ...
            # month year
            filtered_df = filtered_df[
                filtered_df["date"].dt.strftime("%Y-%m") == date_str
            ]
        elif len(parts) == 1:
            # year
...
class NBAKG:
    ...
    def get_games_on_date(self, date_str, basketball_team_name=None):
        ...
        if basketball_team_name is not None:
            ...
            df_game_by_team = pd.read_sql(
                f"select * from game where {team_cond} and {time_cond}", self.conn
            )
            ...
            time_cond = self.get_time_cond(date_str)
            df_game_by_team = pd.read_sql(
                f"select * from game where {time_cond}", self.conn
            )
            if len(df_game_by_team) > 0:
                return df_game_by_team.to_json(date_format="iso")
    ...
    def get_play_by_play_data_by_game_ids(self, game_ids):
        ...
        df_play_by_play_by_gameids = pd.read_sql(
            f"select * from play_by_play where game_id in ({game_ids_str})", self.conn
        )
